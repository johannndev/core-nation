...
.controller('transactionController', function($http, Helper, Barcode, Customer, Item, ariaConstants, $window, $scope) {
var self = this;
self.helper = Helper;
self.scanner = false;
...
//barcode
self.scan = function(index) {
  $('#scannerModal').modal('open');
  startBarcodeScanner();
}

self.closeScanner = function(index) {
  $('#scannerModal').modal('close');
  Barcode.stop();
}
