...
use DB, App\Libraries\Dater;
use App\Models\Transaction;
use App\Models\Account, App\Models\Depreciation;
use App\Models\Settings, App\Models\MonthlyRecord;
use App\Models\Customer, App\Models\CustomerClass, App\Models\Operation;
...
class RecordManager extends BaseManager
{
	...
	{
		$date = Dater::createFromFormat('m/Y',"$month/$year");

		$validator = \Validator::make(array('date' => Dater::toSQL($date->format(Dater::$format))), array('date' => 'cdate'));
		if(!$validator->fails())
			return true;
	...
	{
		$date = Dater::createFromFormat('j/m/Y',"1/$month/$year");
		...
		if(!$monthly->save())
			return $this->error('cannot save monthly data');
	...
	public function getProfitLoss($month, $year)
	{
		...
		if(!$this->checkDate($month,$year))
			return $this->error('invalid month');
