...
class Deleter extends BaseManager
{
	protected $d;//new
	protected $t;
	protected $sm;
	...
	public function delete($transaction)
	{
		...
		if(!$this->d->save())
		{
			$msg = array_flatten($this->d->getErrors());
		...
		foreach($details as $detail)
		{
			...
			if($item->type == Item::TYPE_ASSET_TETAP)
			{
				...
				if($dep)
				{
					...
					$expire = Dater::fromSQL($detail->date)->addMonths(intval($dep->value));
					$dep->expire_date = Dater::toSQL($expire->format(Dater::$format));
					if(!$dep->save())
			...
			ItemsManager::add($this->t->sender_id, $detail->item, $detail->quantity);
			ItemsManager::deduct($this->t->receiver_id, $detail->item, $detail->quantity, false);
	...
	protected function delete_sell()
	{
		...
		foreach($details as $detail)
		{
			...
			ItemsManager::add($this->t->sender_id, $detail->item, $detail->quantity);
			ItemsManager::deduct($this->t->receiver_id, $detail->item, $detail->quantity, true);
	...
	protected function delete_move()
	{
		...
		foreach($details as $detail)
		{
			...
			ItemsManager::add($this->t->sender_id, $detail->item, $detail->quantity);
			ItemsManager::deduct($this->t->receiver_id, $detail->item, $detail->quantity, $can_minus);
	...
	protected function delete_production()
	{
		...
		foreach($details as $detail)
		{
			...
			ItemsManager::deduct($this->t->receiver_id, $detail->item, $detail->quantity);
