...
class ItemsManager
{
	...
	protected function createCrystalItem($group, $input, $tags, $type_id, $size_id, $item = false)
	{
		if(!$item)
			$item = new Item($input);
	...
	public function updateItem($id, $input, $inputTags, $file = null)
	{
		...
		//then create the items
		if(!$item = $this->createCrystalItem($item->group, $input, $inputTags, $type_id, $size_id, $item))
			return $this->error('error creating item');
