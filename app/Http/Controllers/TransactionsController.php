...
class TransactionsController extends Controller
{
	...
	public function getDetail($id, Request $request)
	{
		...
		$data = Transaction::with(['receiver', 'sender', 'user', 'submitByA', 'submitByB', 'transactionDetail', 'transactionDetail.item', 'transactionDetail.item.group', 'destyReceiver', 'destySender'])->where('id', $id)->first();
	...
	public function destyExport(Request $request, $id)
	{
		...
		if ($data->type == Transaction::TYPE_MOVE) {
			if ($request->side == "A") {
				...
				$filename = $typeName . '-' . $data->invoice . '-Receiver.xlsx';
			} else {
