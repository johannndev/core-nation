...
//		$ccs->class = $this->getClass($ccs->rating);
	...
	public function getStat($id,$from = null,$to = null)
	{
		...
		if(!$from || !$to)
		{
			$from = DateHelper::toSQL(Carbon::now()->subMonths(2)->startOfMonth()->format(DateHelper::$format));
			$to = DateHelper::toSQL(Carbon::now()->endOfMonth()->format(DateHelper::$format));
		}
