<x-layouts.layout>
                    ...
                    <table class="w-full print:table-fixed text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead
                            ...
                            <tr>
                                ...
                                <th scope="col"
                                    class="px-4 py-3 print:px-0 print:py-0 print:break-words print:text-wrap">Jubelio
                                </th>
                            ...
                            @forelse ($data->transactionDetail as $itemTd)
                                ...
                                <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td
                                        ...
                                        @if (is_null($status))
                                            <span
                                                class="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-gray-700 dark:text-gray-300">Belum
                                                Cek</span>
                                        @elseif ($status === 0)
                                            <span
                                                class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-red-900 dark:text-red-300">Tidak
                                                Ada</span>
                                        @elseif ($status > 0)
                                            <span
                                                class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">{{ $status }}</span>
                                        @endif
    ...
    @if ($warehouses['sender'])
        ...
        @php
            ...
            $symbol = $adjustType == 'add' ? '+' : '-';
            $color = $adjustType == 'add' ? 'text-green-500' : 'text-red-500';
        @endphp
        ...
        <div class="p-4 mb-4 text-blue-800 border border-blue-300 rounded-lg bg-blue-50" role="alert">
            ...
            <div class="mt-2 mb-4 text-sm">
                <p>
                    Adjustment stok sebanyak
                    <span class="font-bold {{ $color }}">
                        {{ $symbol }}{{ $data->total_items }}
                    </span>

                    pada warehouse
                    <span class="font-bold">
                        {{ $warehouses['sender']['system_name'] }}
                    </span>
                    di Desty

                    @if ($data->a_submit_by)
                        - Disubmit oleh
                        <span class="font-bold">{{ $data->submitByA->username }}</span>,
                        Adjustment ID
                        <span class="font-bold">{{ $data->a_reference_id }}</span>
                    @endif
                </p>
            </div>
    ...
    <div class="p-4 mb-4 text-blue-800 border border-blue-300 rounded-lg bg-blue-50" role="alert">
        ...
        <div class="mt-2 mb-4 text-sm">
            <p>
                ...
                pada warehouse
                <span class="font-bold">
                    {{ $warehouses['receiver']['system_name'] }}
                </span>
                di Desty

                @if ($data->b_submit_by)
                    - Disubmit oleh
                    <span class="font-bold">{{ $data->submitByB->username }}</span>,
                    Adjustment ID
                    <span class="font-bold">{{ $data->b_reference_id }}</span>
                @endif
            </p>
        </div>
