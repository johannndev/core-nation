<x-layouts.layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mb-6">
        ...
        <p class="text-2xl font-bold">Edit Cron {{$data->command}}</p>
    ...
    <form action="{{route('cronrunner.update',$data->id)}}" method="post" >
                    ...
                    <div class="">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-8">
                            <div>
                                ...
                                <input type="text" id="disabled-input" aria-label="disabled input" class=" bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $data->name }}" disabled>
                            </div>

                            <div>
                                <label for="schedule" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">schedule</label>
                                <select id="schedule" name="schedule" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    <option {{old('schedule',$data->schedule) == 'everyMinute' ? 'selected' : ''  }} value="everyMinute">Every Minute</option>
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    <option {{old('status',$data->status) == 1 ? 'selected' : ''  }} value="1">Running</option>
                                    <option {{old('status',$data->status) == 0 ? 'selected' : ''  }} value="0">Stopped</option>
                                </select>
                            </div>
